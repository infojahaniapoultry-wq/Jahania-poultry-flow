  ...
  return (
    <AppLayout>
      <section className="mb-8 flex flex-col justify-between gap-5 md:flex-row md:items-end">
        <div><div className="mb-3 inline-flex items-center gap-2 rounded-full border border-emerald-100 bg-emerald-50 px-3 py-1.5 text-[10px] font-black uppercase tracking-[.16em] text-emerald-800"><span className="h-1.5 w-1.5 animate-pulse rounded-full bg-emerald-500" />Live operations</div><h1 className="text-3xl font-black tracking-[-.04em] text-slate-950 sm:text-4xl">{user?.role === 'ADMIN' ? 'Good morning, Admin.' : 'Welcome to your workbench.'}</h1><p className="mt-2 max-w-xl text-sm leading-6 text-slate-500">A clear pulse of stock, money, and movement across your poultry business.</p></div>
        <Link href="/purchases" className="btn btn-primary self-start md:self-auto"><ShoppingCart size={17} />New transaction</Link>
      </section>
      ...
      <section className="mb-8"><div className="mb-3 flex items-center justify-between"><h2 className="eyebrow">Common actions</h2><span className="text-xs font-bold text-slate-400">Move faster with one tap</span></div><div className="grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-5">{quickActions.map((item) => { const Icon = item.icon; return <Link key={item.href} href={item.href} className="group relative flex min-h-[132px] flex-col justify-between overflow-hidden rounded-2xl border bg-white p-4 text-left shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:border-emerald-400 hover:shadow-lg dark:bg-[var(--bg-card)]"><div className="flex items-start justify-between"><div className={`flex h-10 w-10 items-center justify-center rounded-xl border ${item.color} transition-transform duration-300 group-hover:scale-110`}><Icon size={20} /></div><ArrowRight size={15} className="text-slate-300 transition-all group-hover:translate-x-0.5 group-hover:text-emerald-600" /></div><span className="text-sm font-black tracking-tight text-slate-800 transition-colors group-hover:text-emerald-700 dark:text-slate-100">{item.label}</span></Link>; })}</div></section>

      <div className="mb-8 grid grid-cols-1 gap-6 xl:grid-cols-[1.25fr_1fr]">
